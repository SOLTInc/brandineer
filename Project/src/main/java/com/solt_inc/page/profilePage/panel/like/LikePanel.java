package com.solt_inc.page.profilePage.panel.like;
...
public class LikePanel extends Panel {
    ...
    public LikePanel(String id, IModel<Integer> userId) {
    ...
    private boolean updateFollow(int userId) {
    	...
    	if (followDao.checkFollower(followEntity.getUserId(),followEntity.getFollowId())) {
    		//UPDATE
    		updateFlg = followDao.update(followEntity,2);
    	}else {
    		//INSERT
    		updateFlg = followDao.insert(followEntity,2);
    	}
